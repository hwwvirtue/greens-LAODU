...
/**
  ...
  * @retval None
  */
void forward_Turn(fp32 getAnglenow2,float angle,float gospeed)
{
  float getAngle=0.0f;
  float A_speed=0.0f;
  float AKp=180,AKi=0,AKd=0;
  Angle_PidPara(AKp,AKi,AKd);
  ...
  * @retval None
  */
void back_Turn(fp32 getAnglenow3,int getPos[3],float angle,float gospeed)
{
  float getAngle=0.0f;
  float A_speed=0.0f;
  ...
  else    //未到达目标直线距离范围内时，用前进转弯来接近目标直线
  {
    if((straightLinePara1._B1 > -0.005f) && (straightLinePara1._B1 < 0.005f))
    {
      if(!dir)
      {
        ...
        forward_Turn(getAngleNow,setAngle+angleAdd,setSpeed);
      }
      else
      {
        if(straightLinePara1.A1 > 0)
        {
          setAngle=-180;      //目标角度为竖直直线 方向由dir确定 0为上 1为下
          forward_Turn(getAngleNow,setAngle-angleAdd,setSpeed);//角度PiD转向目标角度
        }
        else
        {
...
void closeRound(getpos getPos1,float x,float y,float R,float clock,float v)
{
  ...
  float disSet=sqrt(pow(getPos1.X-x,2)+pow(getPos1.Y-y,2))-R;
  k=(getPos1.X-x)/(y-getPos1.Y);    //前进直线斜率

  float AKp=0.8f,AKi=0.0f,AKd=0.0f;
  Angle_PidPara(AKp,AKi,AKd);
...
//  顺1逆2
  if(clock==1)
  {
    if(getPos1.Y>y)
      ...
      Agl=90+atan(k)*180/Pi;
    else if(getPos1.Y==y&&getPos1.X>=x)
      Agl=180;
    else if(getPos1.Y==y&&getPos1.X<x)
      Agl=0;
    float disNow=0;
    setangle=Agl+DistancePid(disSet, disNow);//距离赋值给角度的增量
    A_speed=AnglePid(setangle,getPos1.ANG);//角度PID计算两轮速度差值
  }
  else if(clock==2)
  {
    if(getPos1.Y>y)
